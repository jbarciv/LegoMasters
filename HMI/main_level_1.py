...
Description: This script launches a HMI where to decide whether to replicate an own construction (2D) or to build some backup constructions.
...
root.title("HMI - Level 1")
root.geometry("1290x1000")

title = Label(root, text="Camera", font=('Arial', 20))
title.grid(row=0, column=0)
replica_label = Label(root, text="Image analysis", font=('Arial', 20))
...
build_button = Button(capture_frame, text="Build!", command=lambda: build(0, None))
camera_button.grid(row=0, column=0, pady=1)  
capture_button.grid(row=0, column=1, pady=1, padx=10)
build_button.grid(row=0, column=2, pady=1, padx=10)

replica_label = Label(root, text="Replicates other example constructions", font=('Arial', 20))
replica_label.grid(row=3, column=0)

backup_folder = "./backup_nivel_1"
