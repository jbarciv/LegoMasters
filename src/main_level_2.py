...
Description: This script launches an HMI to decide whether to replicate an existing 3D construction or to build backup constructions.
...
from take_photos import come_from_zone_1, come_from_zone_2, go_zone_1, go_zone_2
...
ZONES = {        
    ...
    5: [273.45,-82.67,-111.57,-75.5,89.78,338.43],
    6: [273.1,-90.34,-104.46,-74.95,89.8,338.07],
    7: [272.8,-98.03,-96.14,-75.57,89.83,337.77],
...
def capture_image(which):
    global whereIAm, captured, which2analyze
    if whereIAm["camera_1"] == True and which == 1:
        captured = True
        which2analyze = which
        print("Image 1 captured")
    elif whereIAm["camera_2"] == True and which == 2:
        captured = True
        which2analyze = which
        print("Image 2 captured")
    else:
        print("First move to the correct zone please")

def analyze_image(frame):
    global num, which2analyze
    images_path = "./tmp"
...
def build(build_num, json_path):
    global num, building, backup_folder, tmp_folder
    building = True
    go_to_loading()
    ...
    if build_num == 0:
        ...
        for j in range(1,5):
            for i in range(1,8):
                # print(i,j)
                for my_lego in legos:
                    if ( (my_lego[0] == i) and (my_lego[1] == j)):
                        my_legos_ord.append(my_lego)
                        print(my_lego)
        with open(new_folder + "/mix.json", 'w') as f:
            json.dump(my_legos_ord, f)
        json_path = new_folder + "/mix.json"
    else:
    ...
    for lego in legos:
        print(lego)
        columns.append(lego[0])
        rows.append(lego[1])
        colors.append(lego[2])
    zones, colors = planner(columns, rows, colors)
    print(zones)
    pieces = load_planner(colors)
    print(pieces)
    for selected_piece, selected_zone in zip(pieces, zones):
        piece = BLOCKS[selected_piece]["Joints"]
        zone = ZONES[selected_zone]
        take_block(piece, rtde_c, rtde_r)
        build_block(zone, rtde_c, rtde_r)

    building = False

def go_home():
    print("Going home")
    if whereIAm["ready_build"]:
        rtde_c.moveJ([1.59, -1.538, -0.06, -1.56, 0.014, 1.18], 1, 0.6)
        whereIAm["ready_build"] = False
    elif whereIAm["camera_1"]:
        come_from_zone_1()
        whereIAm["camera_1"] = False
    elif whereIAm["camera_2"]:
        come_from_zone_2()
        whereIAm["camera_2"] = False
    whereIAm["home"] = True
...
def go_to_zone_1():
    ...
    if whereIAm["home"]:
        whereIAm["home"] = False
    elif whereIAm["ready_build"]:
        go_home()
        whereIAm["ready_build"] = False
    elif whereIAm["camera_2"]:
        come_from_zone_2(rtde_c)
        whereIAm["camera_2"] = False
    go_zone_1(rtde_c)
    whereIAm["camera_1"] = True

def go_to_zone_2():
    print("Going to zone 2")
    if whereIAm["home"]:
        whereIAm["home"] = False
    elif whereIAm["ready_build"]:
        go_home()
        ...
        come_from_zone_1(rtde_c)
        whereIAm["camera_1"] = False
    go_zone_2(rtde_c)
    whereIAm["camera_2"] = True

def go_to_loading():
    print("Going to loading zone")
    if whereIAm["home"]:
        whereIAm["home"] = False
    elif whereIAm["camera_1"]:
        come_from_zone_1(rtde_c)
        ...
        come_from_zone_2(rtde_c)
        whereIAm["camera_2"] = False
    zona = [130,-90.95,-100.17,-78.88,90.02,194.98]
    cerca_piez_radian = [math.radians(i) for i in zona]
    rtde_c.moveJ(cerca_piez_radian, 1, 0.6)
    whereIAm["ready_build"] = True
...
capture_1_button = Button(capture_frame, text="Take and Analyze 1", command=lambda: capture_image(1))
capture_2_button = Button(build_frame, text="Take and Analyze 2", command=lambda: capture_image(2))
build_button = Button(build_frame, text="Build!", command=lambda: build(0, None))
camera_button.grid(row=0, column=0, pady=1)  
capture_1_button.grid(row=0, column=0, pady=1)
capture_2_button.grid(row=0, column=0, pady=1)
build_button.grid(row=0, column=2, pady=1)

replica_label = Label(root, text="Replicates other example constructions", font=('Arial', 20))
replica_label.grid(row=3, column=1)

backup_folder = "./backup_nivel_2"
demos_folder = backup_folder+"/demos"

demos_files = glob.glob(os.path.join(demos_folder, "*.png"))
demos_files.sort()

image2load = [] 
for image_path in demos_files:
...
camera_button_1 = Button(camera_frame, text="Build", command=lambda: build(1, demos_files[0]))
camera_button_2 = Button(camera_frame, text="Build", command=lambda: build(2, demos_files[1]))
camera_button_3 = Button(camera_frame, text="Build", command=lambda: build(3, demos_files[2]))
...
capture_button_4 = Button(capture_frame, text="Build", command=lambda: build(4, demos_files[3]))
capture_button_5 = Button(capture_frame, text="Build", command=lambda: build(5, demos_files[4]))
capture_button_6 = Button(capture_frame, text="Build", command=lambda: build(6, demos_files[5]))
...
build_button_7 = Button(build_frame, text="Build", command=lambda: build(7, demos_files[6]))
build_button_8 = Button(build_frame, text="Build", command=lambda: build(8, demos_files[7]))
build_button_9 = Button(build_frame, text="Build", command=lambda: build(9, demos_files[8]))
...
close_button = Button(camera_frame, text="CLOSE ALL", command=close_all, bg='#CD5C5C', width=15, height=2)
close_button.grid(row=2, column=0, padx=10, pady=30)

capture_1_button = Button(capture_frame, text="Go zone 1", command=go_to_zone_1, bg='lightblue', width=15, height=2)
capture_1_button.grid(row=2, column=0, padx=10, pady=30)
capture_2_button = Button(capture_frame, text="Go zone 2", command=go_to_zone_2, bg='lightblue', width=15, height=2)
capture_2_button.grid(row=2, column=1, padx=10, pady=30)
ready_zone_button = Button(capture_frame, text="Go loading zone", command=go_to_loading, bg='lightblue', width=15, height=2)
ready_zone_button.grid(row=2, column=2, padx=10, pady=30)

home_button = Button(build_frame, text="GO HOME", command=go_home, bg='lightgreen', width=15, height=2)
home_button.grid(row=2, column=2, padx=10, pady=30)
